...
def plot_drr(
        ...
        ticks: Union[bool, None] = True,
        axs: Union[matplotlib.axes._axes.Axes, None] = None,
):
...
def animate(
        out: Union[str, pathlib.Path],  # Savepath
        df: pandas.DataFrame,
        drr: DRR,
        ground_truth: Union[torch.Tensor, None] = None,
        verbose: bool = True,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for idx, row in itr:
            ...
            idxs.append(idx)
        frames = np.stack(
            [imageio.v3.imread(f"{tmpdir}/{idx}.png") for idx in idxs],
            axis=0,
        )

    # Make the animation
    return imageio.v3.imwrite(out, frames, **kwargs)
