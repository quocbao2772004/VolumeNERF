...
class DRR(nn.Module):
    ...
    def __init__(
            ...
            width: Union[int, None] = None,  # Width of the rendered DRR (if not provided, set to `height`)
            dely: Union[float, None] = None,  # Y-axis pixel size (if not provided, set to `delx`)
            p_subsample: Union[float, None] = None,  # Proportion of pixels to randomly subsample
...
def forward(self: DRR):
    ...
    img, alphas, voxels = siddon_raycast(source, target, self.volume, self.spacing)
    return self.reshape_transform(img, batch_size=len(self.rotations)), alphas, voxels, source, target
